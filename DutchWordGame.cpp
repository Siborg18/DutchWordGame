...
bool userKnowsWord(std::string dutchWordIn, std::string englishWordIn, 
                  std::string userInput, bool& gameIsOn, bool& saveFile)
{
  char c{};
  if(userInput.size() == 1)
  {
  ...
  {
    emptySaveFile();
    readLinesFromFile("DutchWordsDefault.txt");
    saveFile = false;
    gameIsOn = true;
    return false;
  }
...
int main ()
{
  ...
  std::cout << "Do you know the word: " << currentWord.first << '\n' << "(yes or no)(reset to restart game or exit to quit) "  << std::endl;
  std::cin >> userInput;
  ...
  n = getRandomNumber(wordList.size(), randomSeed());
  }
